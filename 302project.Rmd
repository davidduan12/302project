...
# Introduction
The used car market is expanding rapidly, driven by an increasing number of individuals showing interest in these vehicles. Several factors influence the prices of used cars, and understanding these factors is essential for anyone involved in this market. Whether you are a buyer or a seller, proper analysis is required to make informed decisions in this growing market. “CITE literature”

In this report, we employ a linear regression model to analyze collected data, providing stakeholders with a reliable measure for predicting the expected price of used cars. By delving into the intricacies of the model and its findings, buyers and sellers will gain a deeper understanding of the factors driving price variations, enabling them to make educated decisions.

The response variable $(y_i)$ is the Price of Used Cars, a continuous random variable that follows an approximately normal distribution. The predictor variables $(x_i)$ include the following numerical variables:
...
Power
Seats
Additionally, there are two categorical variables:

Fuel Type
Transmission
A linear regression model can accommodate both numerical and categorical variables. Our analysis revealed that there is minimal skewness in both the response and predictor variables. Moreover, the normality and linearity assumptions required for a linear regression model are met. Based on the fitted model, the coefficients show how changes in a single predictor, while keeping all others constant, affect the price of used cars. Therefore, using a linear regression model to forecast the expected price of a used car is an effective method for predicting prices. “CITE LITERATURE”
