...
```{r echo=FALSE, eval=TRUE, results=FALSE, message=FALSE, warning=FALSE,fig.width=7, fig.height=4}
library(ggplot2)
library(reshape)
...
```{r echo=FALSE, eval=TRUE, results='hide', message=FALSE, warning=FALSE,fig.height=5}
par(mfrow=c(2,4))
...
par(mfrow=c(1,1))
```
Rresults show that our treatment of  removing outliers with cook's distance, using box-cox transformation, and utilizing VIF to remove useless variables improves our model effectively. Despite the residual-fitted value plots still exhibits a slightly cubic polynomial pattern, it is now much flatter and thus random compared to the inital residual value plot of our original model.
